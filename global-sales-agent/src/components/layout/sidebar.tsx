...
export function Sidebar({ appName, pending }: { appName: string; pending: number }) {
  ...
  const item = (n: { key: string; href: string; icon: React.ElementType; match?: string }) => {
    ...
    const Icon = n.icon;
    return (
      <Link key={n.key} href={n.href} className={cn("group flex items-center gap-2.5 rounded-md px-2.5 py-1.5 text-[13px] transition-colors", active ? "bg-muted font-medium text-foreground" : "text-muted-foreground hover:bg-muted/70 hover:text-foreground")} data-testid={`nav-${n.key}`}>
  ...
  return (
    <aside className="hidden w-[232px] shrink-0 flex-col border-r border-border bg-sidebar lg:flex">
      <div className="flex h-14 items-center gap-2 px-4">
        <div className="flex size-7 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Sparkles className="size-3.5" /></div>
        <span className="truncate text-[12px] font-semibold uppercase tracking-[0.16em]" data-testid="app-name">{appName}</span>
      </div>
